...
\showrevisionsfalse
\DeclareRobustCommand{\rev}[1]{{\ifshowrevisions\color{blue}\fi#1}}
\newenvironment{revision}{\begingroup\ifshowrevisions\color{blue}\fi}{\endgroup}
\makeatletter
\expandafter\def\csname revequation\endcsname{%
  \csname equation*\endcsname\ifshowrevisions\color{blue}\fi}
\expandafter\let\csname endrevequation\expandafter\endcsname
  \csname endequation*\endcsname
\expandafter\def\csname revalign\endcsname{%
  \csname align*\endcsname\ifshowrevisions\color{blue}\fi}
\expandafter\let\csname endrevalign\expandafter\endcsname
...
\shorttitle{Distance and resistance on random series--parallel graphs}
\shortauthors{Ding et~al.}

\title[mode=title]{Distance and resistance on random series--parallel graphs:
logarithmic speeds and near-critical asymptotics}

\author[1,2]{Ruiqi Ding}
\ead{dingruiqi@amss.ac.cn}
% \credit{Conceptualization, Methodology, Formal analysis,
% Writing -- original draft, Writing -- review and editing}

\author[1,2]{Zehua He}
\cormark[1]
\ead{hezehua@amss.ac.cn}
% \credit{Conceptualization, Methodology, Formal analysis,
% Software, Writing -- original draft, Writing -- review and editing}

\author[1,2]{Yutao Liang}
\ead{liangyutao@amss.ac.cn}
% \credit{Conceptualization, Methodology, Formal analysis,
% Writing -- review and editing}

\author[3]{Yushu Zheng}
\ead{yszheng666@gmail.com}
% \credit{Conceptualization, Methodology, Formal analysis,
% Writing -- review and editing}

\affiliation[1]{
  organization={State Key Laboratory of Mathematical Sciences, Academy of Mathematics and Systems Science, Chinese Academy of Sciences},
  addressline={No.~55 Zhongguancun East Road, Haidian District},
  city={Beijing},
  postcode={100190},
  country={China}
}
...
\affiliation[2]{
  organization={School of Mathematical Sciences, University of Chinese Academy of Sciences},
  addressline={No.~19A Yuquan Road, Shijingshan District},
  city={Beijing},
  postcode={100049},
  country={China}
}
...
\affiliation[3]{
  organization={School of Mathematics, Shanghai University of Finance and Economics},
  addressline={No.~777 Guoding Road, Yangpu District},
  city={Shanghai},
  postcode={200433},
  country={China}
}
...
obtained by Chen, Derrida, Duquesne, and Shi (2026). \rev{A companion Lean~4
development formalizes the three principal theorems and their proof graph,
subject to two explicitly documented external mathematical assumptions.}
\end{abstract}
...
\sep graph distance
\sep effective resistance
\sep logarithmic speeds
\sep near-critical asymptotics
...
$0<p<1/2$. \rev{The plotted global continuation is arbitrary and is not
computed data.}}
...
\rev{To prove Theorem~\ref{thm:near-critical-speed}, we construct two families of random variables that yield upper and lower bounds for $\log R_n(\frac12+\delta)$. Consider the upper bound, for example. Let $\mathcal T_{\frac12+\delta}$ denote the one-step CDF
...
small \(\varepsilon\), we construct \rev{a} random variable
\(Y_\varepsilon\) with CDF \(G_\varepsilon\)\rev{ such that}
...
\end{align}
\rev{Here $F_0(x)=\mathbf 1_{\{x\ge0\}}$ is the CDF of $\log R_0(\frac12+\varepsilon^3)$. The first inequality in \eqref{prop_G} gives $\log R_0(\frac12+\varepsilon^3)
...
\(G_\varepsilon(x-\kappa\varepsilon^2)-G_\varepsilon(x)\) caused by translation.
 \rev{Matching their leading-order coefficients yields the}
...
\rev{directly from the above comparison between the CDF update and the translation.} Test the operator on a slowly varying CDF
...
\begin{revision}
\subsection*{AI-assisted formalization methodology}
During August 2026, the authors used OpenAI ChatGPT with the GPT-5.6 model
identified in the project record and OpenAI Codex with a GPT-5-family model
(the client did not expose a more specific deployment identifier). The tools
assisted with initial proof sketches, Lean declaration and proof drafts,
mathlib search, consistency checks, language editing, and suggestions for the
manuscript--formalization crosswalk. They did not serve as authors or as
sources of mathematical authority.

The human authors specified the mathematical and graph-semantic contracts,
reconstructed and checked the arguments, verified literature claims against
primary sources, reviewed the generated Lean source, and accepted or rejected
each substantive change. Reproducibility checks use Lean~4.32.1 and mathlib
revision \texttt{520045ab14e26149ee970e2e617ca04b09bde5d6}; they include
filewise kernel checking, a clean \texttt{lake build}, proof-escape and
placeholder scans, and \texttt{\#print axioms} fingerprints. The frozen
artifact records the specifications, crosswalk, build instructions, audit
reports, and two explicit external mathematical interfaces forming the trust
boundary. Archive identifiers are given in the Code availability section.
\end{revision}
...
\end{aligned}
\]
\rev{This} proves the formula for $S_F$.
...
\begin{equation*}
\rev{\int_0^\infty\bigl(h(r)^2+rh(r)\bigr)\,\dd r=\zeta(3).}
\end{equation*}
...
Thus $F_{\varepsilon,z_0}$ satisfies the required \rev{inequality} \eqref{upp_barrier_ineq}, with a
...
To bound the remainder, we first derive two estimates on $q$. \rev{Write} $M:=M_\lambda$, and let $C_\lambda$ denote constants that may change from line to line. Since $q>0$, the case $j=1$ in
...
For the \rev{series-composition} map, coordinatewise monotonicity gives
\(
 g_+(Z_1,Z_2)
...
$p_+\le1$, $\varepsilon\le1$, and the equality of the \rev{parallel-composition}
CDFs, we obtain
\[
\begin{aligned}
 0
...
Proposition~\ref{prop:Cstar-halfline}.
  \rev{Combining \eqref{eq:sharp-upper-bound} and
...
have the same initial value \(0\), so \rev{Lemma~\ref{lem:cuberoot-comparison} gives}
\[
...
\(\lambda_0k-1>0\). Choose \rev{a} small \(\eta>0\) such that
\[
...
\begin{revision}
No empirical data sets were generated or analyzed in this study.  The
machine-checkable Lean sources, manuscript--formalization crosswalks, and
verification reports are included in the versioned software release described
below.
\end{revision}

\section*{Code availability}
\begin{revision}
The Lean~4 development formalizing the appendix, the traditional graph
semantics, and the proof graph for the three principal results is versioned at
\url{https://github.com/iyalice/Series-Parallel}.  Its frozen release is
\texttt{v1.0.1}, at commit
\texttt{a661859343939016608333bd2673d433c34518f2}.  The release landing page is
\url{https://github.com/iyalice/Series-Parallel/releases/tag/v1.0.1}, and the
corresponding source archive is
\url{https://github.com/iyalice/Series-Parallel/archive/refs/tags/v1.0.1.zip}.
The release contains the manuscript--formalization source crosswalk, build and
axiom reports, reproduction instructions, and the explicitly documented
external mathematical interfaces that form the stated trust boundary.  It is
checked with Lean~4.32.1 and mathlib~v4.32.1 at commit
\texttt{520045ab14e26149ee970e2e617ca04b09bde5d6}, and it is distributed under
the Apache-2.0 license.  The repository is currently access-controlled, and no
DOI has been assigned to this release.  The software citation is
\cite{DingHeLiangZheng2026Software}.
\end{revision}

\section*{Declaration of generative AI and AI-assisted technologies\\
in the manuscript preparation process}
\begin{revision}
During the preparation of this work, the authors used OpenAI ChatGPT with the
GPT-5.6 model identified in the project record and OpenAI Codex with a
GPT-5-family model (the client did not expose a more specific deployment
identifier).  These tools assisted with initial mathematical proof sketches,
Lean declaration and proof drafts, mathlib search, consistency checks,
language editing, and suggestions for the manuscript--formalization
crosswalk.  The authors subsequently reconstructed, checked, and revised the
mathematical arguments; verified the references; reviewed the Lean source and
its documented axiom dependencies; and validated the manuscript--formalization
correspondence.  The authors made all final mathematical and editorial
decisions and take full responsibility for the content of the article.
\end{revision}
...
\rev{[software] R.~Ding, Z.~He, Y.~Liang, and Y.~Zheng,}
\rev{\emph{Lean~4 formalization for Distance and resistance on random
series--parallel graphs}, version 1.0.1, 2026.}
\rev{\url{https://github.com/iyalice/Series-Parallel/releases/tag/v1.0.1}}
