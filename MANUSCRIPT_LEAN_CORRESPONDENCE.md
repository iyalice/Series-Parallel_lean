...
Authoritative manuscript source: `Distance_and_resistance_arxiv_amsart.tex`. The table follows its
printed numbering. It contains all 29 theorem, proposition, and lemma environments: 20 in the main
text and 9 in the appendices. TeX labels and TeX line numbers are intentionally confined to
...
| Lemma 4.1 | `integral_diffusionIntegrand_eq_zetaThree`; `diffusionCoefficient_eq_two_mul_zetaThree` | exact; the second declaration is the derived normalization $a=2\zeta(3)$ |
| Proposition 4.2 | `full_line_distribution`; `full_line_density_probability` | exact |
| Proposition 4.3 | `hard_edge_distribution`; `hard_edge_density_probability` | exact |
| Lemma 4.4 | `full_line_weighted_consistency` | exact |
| Lemma 4.5 | `affineWaveProfile_cdfOperator_le_upperBarrier_add_error` | the estimate used in Theorem 1.3 is proved; the complete displayed lemma is not exported as one declaration |
| Lemma 4.6 | `hard_edge_weighted_lower_consistency` | exact |
| Proposition 4.7 | `hard_edge_global_lower_barrier` | exact |
...
| Lemma A.2 | `shootingProperties`; `shootingYMap_norm_sub_le` | exact, including the parameter Lipschitz estimate |
| Proposition A.3 | `cstarHalfline`; `existsUnique_boundarySolution_iff_lambdaStar_le`; `lambdaLower_eq_rpow` | exact |
| Proposition A.4 | `subcriticalW` | exact |
| Lemma A.5 | `linearAtOne`; `WSolution_linearAtOne` | exact |
| Lemma A.6 | `leftEndpointDichotomy_source` | exact |
| Proposition A.7 | `criticalBranches` | exact |
...
The qualification for Lemma 4.5 does not affect the closure of any of Theorems 1.1--1.3.
